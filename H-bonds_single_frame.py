...
for j in range(1, 65):
   hlist = []
   Ohbond = 0
   OXY_A = [oxygens[(i * 64) + j - 1][2], oxygens[(i * 64) + j - 1][3], oxygens[(i * 64) + j - 1][4]]
#   oxy_range = range(1, 65)
   oxy_range = range(1, len(oxygens) + 1)
   oxy_range.remove(j)
   for k in range(1, 3):
      HYD_A = [hydrogens[(i * 128) + (2 * j) + k - 3][3], hydrogens[(i * 128) + (2 * j) + k - 3][4], hydrogens[(i * 128) + (2 * j) + k - 3][5]]
      for l in oxy_range:
         OXY_B = [oxygens[(i * 64) + l - 1][2], oxygens[(i * 64) + l - 1][3], oxygens[(i * 64) + l - 1][4]]
         A = [OXY_A, HYD_A, OXY_B]
   ...
   Ohbond_file.write("%i\t%i\n" % (j, Ohbond))
            
Ohbond_file.seek(0, 0)
